<template>
  <div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="card dark:card-dark p-4 border-l-2 border-primary-500">
        <div class="flex items-center gap-1.5">
          <span class="h-1.5 w-1.5 rounded-full bg-primary-500"></span>
          <p class="text-xs text-dark-500 dark:text-dark-400 font-medium">仓库总数</p>
        </div>
        <p class="text-2xl font-bold text-dark-900 dark:text-white mt-2 tabular-nums">{{ repositories.length }}</p>
      </div>
      <div class="card dark:card-dark p-4 border-l-2 border-emerald-500">
        <div class="flex items-center gap-1.5">
          <span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
          <p class="text-xs text-dark-500 dark:text-dark-400 font-medium">已启用</p>
        </div>
        <p class="text-2xl font-bold text-dark-900 dark:text-white mt-2 tabular-nums">{{ enabledCount }}</p>
      </div>
      <div class="card dark:card-dark p-4 border-l-2 border-blue-500">
        <div class="flex items-center gap-1.5">
          <span class="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse"></span>
          <p class="text-xs text-dark-500 dark:text-dark-400 font-medium">同步中</p>
        </div>
        <p class="text-2xl font-bold text-dark-900 dark:text-white mt-2 tabular-nums">{{ syncingCount }}</p>
      </div>
      <div class="card dark:card-dark p-4 border-l-2 border-red-500">
        <div class="flex items-center gap-1.5">
          <span class="h-1.5 w-1.5 rounded-full bg-red-500"></span>
          <p class="text-xs text-dark-500 dark:text-dark-400 font-medium">同步失败</p>
        </div>
        <p class="text-2xl font-bold text-dark-900 dark:text-white mt-2 tabular-nums">{{ failedCount }}</p>
      </div>
    </div>
    ...
    <transition name="page">
      <div v-if="showAddForm" class="card dark:card-dark p-6 animate-slide-up">
        <h2 class="text-base font-semibold text-dark-800 dark:text-white mb-4">添加仓库</h2>
    ...
    <div v-else-if="repositories.length === 0" class="card dark:card-dark py-12 text-center">
      <GitBranch class="h-6 w-6 text-dark-300 dark:text-dark-600 mx-auto mb-3" />
      <h3 class="text-sm font-semibold text-dark-700 dark:text-white mb-1">暂无仓库配置</h3>
      <p class="text-dark-400 dark:text-dark-500 text-xs">点击右上角"添加仓库"开始配置</p>
    </div>
      ...
      <div v-for="repo in repositories" :key="repo.id" class="card dark:card-dark p-5 hover:shadow-elevated dark:hover:shadow-dark-elevated transition-shadow duration-300">
        ...
        <div class="flex items-start justify-between mb-3">
          <div class="flex items-center gap-2.5 min-w-0">
            <GitBranch class="h-4 w-4 text-primary-500 flex-shrink-0 mt-0.5" />
            <div class="min-w-0">
              <h3 class="font-semibold text-dark-800 dark:text-white truncate">{{ repo.displayName || repo.name }}</h3>
              <div class="flex items-center gap-1.5 mt-1">
                <span class="badge badge-info dark:badge-info-dark">{{ repo.type }}</span>
                <span :class="repo.enabled ? 'badge badge-success dark:badge-success-dark' : 'badge badge-neutral dark:badge-neutral-dark'">
                  {{ repo.enabled ? '启用' : '禁用' }}
                </span>
              </div>
            </div>
          ...
          <div class="flex gap-0.5 flex-shrink-0">
            <button
              ...
              class="rounded-md p-1.5 text-dark-400 dark:text-dark-500 hover:bg-primary-50 dark:hover:bg-primary-600/10 hover:text-primary-600 dark:hover:text-primary-400 transition-colors disabled:opacity-50"
              title="同步"
            >
              <RefreshCw class="h-3.5 w-3.5" :class="syncingIds.has(repo.id!) ? 'animate-spin' : ''" />
            </button>
            <button
              @click="deleteRepo(repo.id!)"
              class="rounded-md p-1.5 text-dark-400 dark:text-dark-500 hover:bg-red-50 dark:hover:bg-red-600/10 hover:text-red-600 dark:hover:text-red-400 transition-colors"
              title="删除"
            >
        ...
        <div class="space-y-1 text-sm">
          <div class="flex items-center gap-1.5 text-dark-500 dark:text-dark-400">
        ...
        <!-- Sync History -->
        <div v-if="syncHistory[repo.id!]?.length" class="mt-4 pt-3 border-t border-dark-100 dark:border-dark-700">
          <p class="text-xs font-medium text-dark-400 dark:text-dark-500 uppercase tracking-wide mb-2">同步历史</p>
          <div class="space-y-1">
            <div
            ...
            >
              <span
                ...
                :class="{
                  'bg-emerald-500': state.status === 'SUCCESS',
                  'bg-red-500': state.status === 'FAILED',
                  'bg-blue-500 animate-pulse': state.status === 'SYNCING' || state.status === 'PENDING'
                }"
              ></span>
              <span class="font-medium text-dark-600 dark:text-dark-300 min-w-[56px]">{{ state.status }}</span>
